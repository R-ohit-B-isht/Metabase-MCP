...
export class DashboardToolHandlers {
    ...
    async addCardToDashboard(args) {
        ...
        for (const card of cards) {
            if (!card.id || typeof card.col !== 'number' || typeof card.row !== 'number' ||
                ...
                throw new McpError(ErrorCode.InvalidParams, "Each card must have id, col, row, size_x, and size_y properties");
            }
        }
        if (tabs.length > 0) {
            for (const tab of tabs) {
                if (!tab.id || !tab.name || tab.name.trim().length === 0) {
                    throw new McpError(ErrorCode.InvalidParams, "Each tab must have id and non-empty name properties");
                }
            }
        }
        const requestBody = {
        ...
        const result = await this.client.apiCall("PUT", `/api/dashboard/${dashboard_id}/cards`, requestBody);
        return {
            content: [
                {
                    type: "text",
    ...
    async postDashboardQueryExport(args) {
        const { dashboard_id, dashcard_id, card_id, export_format, format_rows = false, pivot_results = false, parameters = [] } = args;
