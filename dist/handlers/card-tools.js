...
export class CardToolHandlers {
    ...
    async moveCards(args) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Successfully moved ${card_ids.length} card(s) to ${collection_id ? `collection ${collection_id}` : `dashboard ${dashboard_id}`}. Result: ${JSON.stringify(result, null, 2)}`,
                },
            ],
        };
    }
    async moveCardsToCollection(args) {
    ...
    async executePivotCardQuery(args) {
    ...
    async searchCardParamValues(args) {
    ...
    async createCardPublicLink(args) {
        const { card_id } = args;
        if (!card_id) {
            throw new McpError(ErrorCode.InvalidParams, "Card ID is required");
        }
    ...
    async deleteCardPublicLink(args) {
        const { card_id } = args;
        if (!card_id) {
            throw new McpError(ErrorCode.InvalidParams, "Card ID is required");
        }
    ...
    async executeCardQueryWithFormat(args) {
    ...
    async getCardQueryMetadata(args) {
        const { card_id } = args;
        if (!card_id) {
            throw new McpError(ErrorCode.InvalidParams, "Card ID is required");
        }
